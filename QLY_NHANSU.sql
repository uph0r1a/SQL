...
SELECT TEN_DUAN, HO_NV, TEN_NV, MA_NHANVIEN, NGAY_THAM_GIA, NGAY_KET_THUC, TEN_PB
FROM NHANVIEN A
    JOIN QUANLY_DUAN C ON A.ID_NhanVien = C.MA_NHANVIEN
    JOIN DU_AN B ON B.MA_DUAN = C.MA_DUAN
    JOIN PHONG_BAN D ON A.PHG = D.MA_PB;

SELECT ID_NHANVIEN, HO_NV, TEN_NV, LUONG, TEN_PB
FROM PHONG_BAN A RIGHT JOIN NHANVIEN B
    ON A.MA_PB = B.PHG

SELECT ID_NHANVIEN, HO_NV, TEN_NV, LUONG, TEN_PB
FROM PHONG_BAN A LEFT JOIN NHANVIEN B
    ON A.MA_PB = B.PHG
...
FROM PHONG_BAN
WHERE TEN_PB LIKE 'UDPM')

SELECT NHAN_VIEN.HO_NV + ' ' +NHAN_VIEN.TEN_NV AS HO_TEN_NHANVIEN, NHAN_VIEN.LUONG, PHONG_BAN.TEN_PB, DU_AN.TEN_DUAN, QUANLY_DUAN.NGAY_THAM_GIA, QUANLY_DUAN.SO_GIO
FROM NHAN_VIEN
    JOIN PHONG_BAN ON NHAN_VIEN.PHG = PHONG_BAN.MA_PB
    JOIN QUANLY_DUAN ON NHAN_VIEN.ID_NHANVIEN = QUANLY_DUAN.MA_DUAN
    JOIN DU_AN DA ON QLD.MA_DUAN = DA.MA_DUAN;

SELECT NV.HO_NV + ' ' + NV.TEN_NV AS HO_TEN, NV.LUONG, DA.TEN_DUAN
FROM NHAN_VIEN NV
    JOIN PHONG_BAN PB ON NV.PHG = PB.MA_PB
    JOIN QUANLY_DUAN QLD ON NV.ID_NHANVIEN = QLD.MA_NHANVIEN
    JOIN DU_AN DA ON QLD.MA_DUAN = DA.MA_DUAN
WHERE PB.TEN_PB = N'Thiết kế' AND QLD.NGAY_THAM_GIA = '2016-01-01';
