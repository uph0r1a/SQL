...
--Create procedure to calculate sum
CREATE PROC SP_TONG
    @SO1 INT,
    @SO2 INT
AS
...
--Set default parameters and edit parameters
...
--Get the number of people for a certain gender
...
BEGIN
    ...
    SELECT @NUM = COUNT(*)
    FROM NHAN_VIEN
    WHERE GIOI_TINH LIKE '%' + @GTINH
    RETURN @NUM
END
DECLARE @TONG INT
EXEC @TONG = SOLUONG_NV 'Nam'
SELECT @TONG AS 'So luong nv nam'
GO

--Get person by id number
CREATE PROC THONGTIN_NV_MA
    @MANV NVARCHAR(9)
AS
BEGIN
    SELECT *
...
END
EXEC THONGTIN_NV_MA '13'
SELECT *
FROM NHAN_VIEN
GO

--Get person by name
CREATE PROC THONGTIN_NV_HOTEN
    @HOTENNV NVARCHAR(30)
AS
BEGIN
    SELECT *
    FROM NHAN_VIEN
    WHERE HO_NV + ' ' + TEN_NV LIKE '%' + @HOTENNV + '%'
END
EXEC THONGTIN_NV_HOTEN N'An'
SELECT *
FROM NHAN_VIEN
GO

--Update or add PHONG_BAN
...
BEGIN
    ...
    BEGIN
        INSERT INTO PHONG_BAN
        VALUES(@MAPHONG, @TENPHONG, @MATRUONGPHONG)
    END
END
GO
EXEC SP_THEMPHONGBAN '100', N'Cong nghe thong tin', NULL
